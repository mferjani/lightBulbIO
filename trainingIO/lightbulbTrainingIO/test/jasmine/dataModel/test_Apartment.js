...
var filename = 'test_Apartment';

describe(filename, function () {
  beforeAll(function () {
    this.ctx = TestApi.createContext(filename);
    this.apartment = TestApi.createEntity(this.ctx, 'Apartment');
  ...
  it('verify Fixture.currentBulb', function () {
    ...
    expect(fixture.currentBulb.id).toEqual(this.smartBulb.id);

    fixture = Fixture.get(this.fixtures[1], 'currentBulb.id');
    expect(fixture.currentBulb).toBeUndefined();
  });

  it('verify Apartment.lightBulbsInUse', function () {
    var apartment = Apartment.get(this.apartment.id, 'lightBulbsInUse');
